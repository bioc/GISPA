...
stackedBarplot = function(gispa.output, feature=1, cpt=1, type="EXP", 
  ...
  #data type 1
  per_DF <- as.matrix(gispa.output[,c(1:3,ncol(gispa.output))])
  ...
  if(feature == 2 | feature == 3){
    per_DF <- as.matrix(gispa.output[,c(4:6,ncol(gispa.output))])
    bgfps <- prop.table(per_DF[,1:3], margin=1)*100
    colnames(bgfps)[1]<-"Ref"
    colnames(bgfps)[2]<-"S1"
    colnames(bgfps)[3]<-"S2"
  ...
  #data type 3
  if (feature == 3){
    per_DF <- as.matrix(gispa.output[,c(7:9,ncol(gispa.output))])
    bgfps <- prop.table(per_DF[,1:3], margin=1)*100
    colnames(bgfps)[1]<-"Ref"
    colnames(bgfps)[2]<-"S1"
    colnames(bgfps)[3]<-"S2"
